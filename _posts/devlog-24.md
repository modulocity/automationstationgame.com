...
As you can see, the island is now full of little bushes and patches of grass, making it a lot more lively. It also means that I can reduce the number of blubber trees without it feeling too barren. This is great because the trees cause several issues with obscuring the player or buildings, so reducing the quantity helps a lot.
...
Yup, arms are back. **Arms** are a building that picks up items from an adjacent building and places items in a different adjacent building. If you're familiar with Factorio, it's essentially an inserter.
...
- Demolishing buildings now refund all items used to make it
